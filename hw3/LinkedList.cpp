...
#include<iostream>
template<class T>
LinkedList<T>::LinkedList(const LinkedList<T>& aList) : headptr(nullptr), itemCount(0) {
    if (aList.headptr == nullptr) {
        return; // If the source list is empty, create an empty list
    }

    // Copy first node
    headptr = new Node<T>(aList.headptr->getItem());
    itemCount = aList.itemCount;

    // Copy rest of the list
    Node<T>* sourcePtr = aList.headptr->next;
    Node<T>* targetPtr = headptr;

    while (sourcePtr != nullptr) {
        targetPtr->next = new Node<T>(sourcePtr->getItem());
...
bool LinkedList<T>::insertSorted(const T& data) {
    ...
    Node<T>* newNode = new Node<T>(data);  // Creates a new node with copy of data


    if (headptr == nullptr || headptr->item.getId() > data.getId()) {
    ...
    Node<T>* current = headptr;
    while (current->next != nullptr && current->next->item.getId() < data.getId()) {
...
template <class T>
void LinkedList<T>::modifyItem(const T& oldItem, const T& newItem) {
    Node<T>* current = headptr;
    while (current != nullptr) {
        if (current->getItem().getId() == oldItem.getId()) {
